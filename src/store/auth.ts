...
export const Auth = new Vuex.Store({
  ...
  actions: {
    async signIn(context, { email, password }) {
      ...
      await axios
        .post(`${auth_v1}/token?grant_type=password&username=${email}&password=${password}`)
        .then((result: any) => {
          if (result.data) {
    ...
    async getSavedVideos(context) {
      ...
      await axios
        ...
        .then((res: any) => {
          if (res?.data?.data) context.commit("setListSavedVideos", res.data.data);
        })
        .catch(function () {
  ...
  getters: {
    ...
    youtubeVideos(state) {
      return state.listSavedVideo.filter(x => (x.source).toLowerCase() == "youtube");
    },
    tikTokVideos(state) {
      return state.listSavedVideo.filter(x => (x.source).toLowerCase() == "tiktok");
    },
    facebookVideos(state) {
      return state.listSavedVideo.filter(x => (x.source).toLowerCase() == "facebook");
    }
  },
});
